...
function setup()
  ...
  return {
    ...
    raster_source = raster:load (
      "raster/UHI.tail.asc",
      150.2682580, -- long min
      151.9062580, -- long max
      -34.7568893, -- lat min
      -32.7228893, -- lat max
      2034, -- num rows
      1638  -- num cols
...
function process_segment (profile, segment)
    ...
    if sourceData.datum ~= invalid and targetData.datum ~= invalid then
        local avg = (sourceData.datum + targetData.datum) / 2

        local penalty = (avg + 3) / 15
        -- +3 to make values positive
        -- values should be between 0 and 15

        scaled_weight = scaled_weight * penalty * 100
    end
